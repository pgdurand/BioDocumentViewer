...
public class DocViewerConfig {
  ...
  /**
   * List the current bank providers.
   */
  public static enum BANK_PROVIDER {
    NCBI, EBI
  };
  ...
  /**
   * JVM optional argument. Provide a bank provider. If not provided, software uses
   * NCBI as default. Sample use: -DV_PROVIDER=EBI
   */
  ...
  private static BANK_PROVIDER DEFAULT_PROVIDER = null;
   ...
   */
  public static BANK_PROVIDER getBankProvider() {
    if (DEFAULT_PROVIDER == null) {
      String provider = System.getProperty(JVM_ARG_BK_PROVIDER);
      if (provider != null) {
        switch (provider.toLowerCase()) {
        case "ebi":
          DEFAULT_PROVIDER = BANK_PROVIDER.EBI;
          break;
        case "ncbi":
        default:
          DEFAULT_PROVIDER = BANK_PROVIDER.NCBI;
          break;
        }
      } else {
        DEFAULT_PROVIDER = BANK_PROVIDER.NCBI;
    ...
    EZLogger.debug("Bank provider is: " + DEFAULT_PROVIDER);
   ...
   */
  public static void setBankProvider(BANK_PROVIDER provider) {
    if (provider == null)
      return;
  ...
  public static void initLogLevel() {
    ...
    // set the Formatter of the UI logger: in debug mode, provide full class/method names
  ...
  /**
   * Return the path where filters targeting particular banks can be saved or loaded.
   * 
  ...
  public static String getFilterStoragePath(BankType bt){
    ...
    f = new File(
        EZFileUtils.terminatePath(System.getProperty("user.home"))+
        "."+
        str+
        File.separator+
        "filters"+
