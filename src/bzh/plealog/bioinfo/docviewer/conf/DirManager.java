 ...
 */
package bzh.plealog.bioinfo.docviewer.conf;
...
public class DirManager extends AbstractPropertiesConfig {
  ...
  /**
   * Get the full path to the directory where the application stores doc
   ...
   *           if the method failed to create the path.
   */
  public String getDocumentDataPath() throws IOException {
    if (_docDataPath != null)
      return _docDataPath;

    _docDataPath = getApplicationDataPath()
        + EZFileUtils.terminatePath(DOC_DIR);

    createPath(_docDataPath);
